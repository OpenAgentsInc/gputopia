...
class AiWorker {
    ...
    try {
      ...
      if (this.stream) {
        this.websocket?.send(
          JSON.stringify({
            model: this.model,
            choices: [{ delta: { role: 'assistant', content: this.streamContent(reply) }, index: 0 }]
          })
        )
        this.websocket?.send(
          JSON.stringify({
            model: this.model,
            choices: [{ delta: { role: 'assistant', content: '' }, finish_reason: 'stop', index: 0 }]
          })
        )
      } else {
        this.websocket?.send(
          JSON.stringify({
            model: this.model,
            choices: [{ message: { role: 'assistant', content: reply }, finish_reason: 'stop', index: 0 }]
          })
        )
      }
    } catch (e) {
  ...
  private async getOpenAiReply(req: any) {
    ...
    if (req.top_p) pipeline.config.top_p = req.top_p

    if (req.temperature) pipeline.config.temperature = req.temperature
    ...
    this.streamStart = 0
    const reply = await this.chat.generate(
  ...
  private progress(report: any, content: string) {
    if (this.stream) {
      this.websocket?.send(
        JSON.stringify({ choices: [{ delta: { role: 'assistant', content: this.streamContent(content) } }] })
      )
    }
  ...
  private connect() {
    ...
    let res: any = null
    this.waitConnect = new Promise(resolve => {
      res = resolve
    })
